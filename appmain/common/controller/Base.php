...
class Base extends Controller
{
    ...
    public function getVisitData($storeId = 0, $num = 0,$color = false)
    {
        $storeId = $storeId ? $storeId : 1;
        //异步实时获取投屏数据
        $dataService = new \app\admin\service\Dataset();
        ...
        $total = $totalModel->where(['store_id' => 1])->sum('person_total');
        $normalDataset['personTotal'] = $total * $count;
        //获取到店顾客年龄比例
        $ageDataset         = $dataService->getAgeDataset(false, $color, $count);
        //获取到店顾客性别比例
        $genderDataset      = $dataService->getGenderDataset(false, $color, TRUE, $count);
        //获取新老客户比例
        $customerDataset    = $dataService->getCustomerDataset(false, $color, TRUE, $count);
        ...
        if ($datas) {
            ...
            foreach ($datas as $key => $value) {
                ...
                $showname = $value['user_id'] > 0 ? ($value['is_admin'] > 0 ? '员工' : '会员').($nickname ? ' ('.$nickname.')' : '') : '游客';
                ...
                $personList[] = [
                    'avatar'        => $avatar,
                    'name'          => $showname,
                    'age'           => ($value['age'] > 0 ? $value['age'] : $value['tempage']).'岁',
...
//             ];
        ...
        $chart = new \app\admin\service\Chart("group", ['访问人次'], $label, $dataset, $color, false);
