...
class Timer extends ApiBase
{
    ...
    public function prize_old()
    {
        ...
        if ($totalNum <= 0) {
            $this->_returnMsg(['errCode' => 1, 'errMsg' => '未配置总中奖用户数量']);
        }
        ...
        if ($time <= $startTime || $time > $endTime) {
            $this->_returnMsg(['errCode' => 1, 'errMsg' => '当前时间:'.date('Y-m-d H:i:s').' 不在整点范围内']);
        }
        ...
        if ($flag) {
            ...
            if ($list) {
                foreach ($list as $key => $value) {
                    ...
                    if ($value && $phone) {
                        ...
                        if ($winId === false) {
                            $this->_returnMsg(['errCode' => 1, 'errMsg' => '数据库执行异常']);
                        }
                        if ($userId) {
                            //真实用户发送短信+小程序通知
            ...
            $this->_returnMsg(['errCode' => 0, 'msg' => '中奖用户手机号:'.$phoneStr.' , 未发送通知手机号：'.$phone1Str]);
        }else{
            $this->_returnMsg(['errCode' => 1, 'errMsg' => $msg]);
        }
    }
...
//         $flag = $timestamp <= $time && $time <= $timestamp + 30;
        ...
        if ($exist) {
            $this->_returnMsg(['errCode' => 1, 'errMsg' => '今日已完成抽奖~~']);
        }
