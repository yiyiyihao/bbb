...
class Chart
{
    ...
    //初始化赋值
    public function __construct($type = false, $legend = NULL, $chartName = NULL, $chartData = NULL, $color = NULL, $legendShow = TRUE)
    {
        $color = $color ? $color : ['rgba(225,85,85,0.8)'];
        //初始化设置tooltip
        //$this->option['tooltip']['formatter']     = "{a} <br/>{b} : {c} ({d}%)";
        if ($type) {
            $this->type($type, $legend, $chartName, $chartData, $color, $legendShow);
        }
    }
     ...
     */
    public function xAxis($data = [], $type = 'category', $boundaryGap = false, $axisLine = [])
    {
        $xAxis = [];
        $xAxis['type'] = $type;
     ...
     * 配置y轴数据
     */
    public function yAxis($type = 'value', $axisLine = [])
    {
        $yAxis = [];
        $yAxis['type'] = $type;
     ...
     * 赋值基础类型属性
     */
    public function type($type, $legend = [], $chartName = [], $chartData = [], $color = [], $legendShow = TRUE)
    {
        switch ($type) {
            case 'bar':
